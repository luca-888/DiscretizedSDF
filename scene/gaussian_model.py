...
from scene.NVDIFFREC import (
    create_trainable_env_rnd, load_env, 
)
...
class GaussianModel:
    ...
    def training_setup(self, training_args: OptimizationParams, scene):
        ...
        self.envmap_scheduler_args = get_expon_lr_func(lr_init=training_args.envmap_lr_init,
    ...
    def save_ply(self, path):
        ...
        attributes = [xyz, f_dc, f_rest, sdfs, inverse_deviation, opacities, 
                      scale, rotation, roughness, albedo, metallic]
    ...
    def reset_opacity(self):
        if self.use_sdf:
            def inv_learnable_sigmoid(o, s):
                ...
                inv_dev = -torch.log(tmp) / s
                return inv_dev
            sdfs_threshold = inv_learnable_sigmoid(torch.ones_like(self.get_sdf)*0.01, torch.ones_like(self.get_sdf)*self.inverse_deviation)
    ...
    def load_ply(self, path):
        ...
        self.inverse_deviation = nn.Parameter(torch.tensor(inverse_deviation[0], dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, 
                              new_sdfs, new_opacities, new_scaling, new_rotation, 
                              new_roughness, new_albedo=None, new_metallic=None):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent,):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, 
                                  new_sdf, new_opacity, new_scaling, new_rotation, 
                                   new_roughness, new_albedo, new_metallic)
